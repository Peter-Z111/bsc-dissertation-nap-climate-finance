...
cfu <- cfu %>%
  left_join(
    cfu_10to24 %>% select(Country, Year, Year.Submitted),
    by = c("Country", "Year")
  )
...
log_miti_control <- feols(
  log_funding_miti ~ treat + vul_imputed_1 + read_imputed_3 + pol_imputed_1 | 
    Country + Year, data = cfu, cluster = "Country"
)
...
cs.est <- att_gt(yname = "funding_ad",
                 gname = "Year.Submitted",
                 idname = "Country_ID",
                 tname = "Year",
...
aggte(cs.est, type = "simple", na.rm = T, bstrap = F)
summary(aggte)
